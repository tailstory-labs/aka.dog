  ...
  async fetch(request: Request) {
    ...
    if (first === 'index') {
      if (wantsJson(request)) {
        const segments = pathname.replace(/^index\/?/, '').split('/').filter(Boolean);
        try {
          const body = buildEnvelope(segments.join('/'), resolveViewByPath(segments).entries);
          return new Response(JSON.stringify(body), {
            headers: { 'content-type': 'application/json', 'cache-control': INDEX_CACHE },
          });
        } catch {
          return new Response(JSON.stringify({ error: 'unknown view', view: segments.join('/') }), {
            status: 404,
